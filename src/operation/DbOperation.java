...
public class DbOperation {
	...
	private static final String password = "ganpati1";
	...
	public static ArrayList<String> showRecordFromConfTable(int conf_id) throws SQLException {
		Connection conn = null;
		Statement st = null;
		try{
			conn = getDBConnection();
			st = conn.createStatement();
			ArrayList<String> _rec=new ArrayList<String>();
			System.out.println(conf_id);
			ResultSet res = st.executeQuery("SELECT * FROM  conf_details where conf_id = " + conf_id );
	          while (res.next()) {
	        	  System.out.println(res.getInt("conf_id") + "\t" + res.getString("name") + "\t" + 
	            		  res.getString("date") + "\t" + res.getString("venue") + "\t" + res.getString("link") + "\t" + res.getString("speakers"));
	          
	        	  _rec.add(String.valueOf(res.getInt("conf_id")));
	        	  _rec.add( res.getString("name"));
	        	  _rec.add(res.getString("date"));
	        	  _rec.add(res.getString("venue"));
	        	  _rec.add(res.getString("link"));
	        	  _rec.add(res.getString("speakers"));
	...
	public static String[] getRecordFromUserConfTable(String userName) throws SQLException {
		Connection conn = null;
		Statement st = null;
		try{
			conn = getDBConnection();
			st = conn.createStatement();
		//ArrayList<Integer> confID=new ArrayList<Integer>();
			ResultSet res = st.executeQuery("select recom_conf from user_recom_conf where user_name = '" + userName +"'" );
			String recom_conf = null;
	          while (res.next()) {
	              
	        	  recom_conf= res.getString("recom_conf");
	              //System.out.println(id + "\t" + name + "\t" + date + "\t" + venue + "\t" + link + "\t" + speakers);
	          }
	          
	          String[] recomID= recom_conf.split(",");
	          return recomID;
		}
		catch (SQLException e) {
	...
	public static long searchNumberOfConf(String searchQuery) {
		Connection conn = null;
		Statement st = null;
		//ArrayList<String[]> results = new ArrayList<String[]>();
		long lrec=0;
